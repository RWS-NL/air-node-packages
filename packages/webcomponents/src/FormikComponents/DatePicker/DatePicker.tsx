...
type DatePickerFormProps = DatePickerProps & FieldProps & Omit<KeyboardDatePickerProps, 'error' | 'name' | 'onChange' | 'value' | 'variant'>;
...
export const DatePicker: FC<DatePickerProps> = componentProps => {
  const {
    field, form, label, minDate, ...props
  } = componentProps as DatePickerFormProps;
  ...
  return (
    <KeyboardDatePicker
      ...
      onChange={(newDate: MaterialUiPickersDate | null) => form.setFieldValue(field.name, newDate?.toDate() || new Date(), true)}
      KeyboardButtonProps={{ 'aria-label': 'change date' }}
      InputAdornmentProps={{ position: 'start', className: css.datePickerIcon }}
      label={label}
      inputVariant='outlined'
      fullWidth autoOk animateYearScrolling
      InputProps={{
        classes: {
          root: css.input,
